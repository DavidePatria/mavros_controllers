...
SlowDown::SlowDown(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private)
	...
	// in px4 avoidance
	trigger_distance_{0.3}
	{
	pose_sub_ = nh_.subscribe("/mavros/local_position/pose", 1, &SlowDown::poseCallback_, this, ros::TransportHints().tcpNoDelay());
	goal_sub_ = nh_.subscribe("/command_trajectory", 1, &SlowDown::trajCallback_, this, ros::TransportHints().tcpNoDelay());
	traj_slow_pub_ = nh_.advertise<trajectory_msgs::MultiDOFJointTrajectoryPoint>("command/trajectory_slow", 1);

	cmdloop_timer_ = nh_.createTimer(ros::Duration(0.01), &SlowDown::cmdloopCallback, this);
	};
...
void SlowDown::trajCallback_(const trajectory_msgs::MultiDOFJointTrajectory& msg) {
	...
	// send first point
	// actual_goal_.position = traj_q_.front();
...
// use a timed callback to verify and publish setpoint. maybe the long queue needs to be stored and manipulated
